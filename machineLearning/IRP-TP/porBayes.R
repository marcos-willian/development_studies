...
for(i in folds){
  ...
  bayesTreinamento = naive_bayes(dataTrain$x, dataTrain$classes)
  
  bayesClasse = predict(bayesTreinamento, newdata = dataTest$x, type = "class")
  
  acuraciaFold[i] = acurracy(dataTest$classes, bayesClasse)
}
