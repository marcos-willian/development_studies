...
for(p in c(1, 2, 5, 20)){
  ...
  modRBF =  treinaRBF(base1$x, base1$classes, p)
  Y = sign(YRBF(xgrid, modRBF))
...
for(p in c(1, 2, 4 , 10)){
  set.seed(0) #Para efeitos de comparação
  modRBF =  treinaRBF(base2$x, base2$classes, p)
  Y = sign(YRBF(xgrid, modRBF))
...
for(p in c(1, 2, 4, 6)){
  set.seed(0) #Para efeitos de comparação
  modRBF =  treinaRBF(base3$x, base3$classes, p)
  Y = sign(YRBF(xgrid, modRBF))
...
for(p in c(1, 5, 10, 30)){
  set.seed(0) #Para efeitos de comparação
  modRBF =  treinaRBF(base4$x, base4$classes, p)
  Y = sign(YRBF(xgrid, modRBF))
